...
from hoopaloo_test.models import Student, Exercise, Test, UnderTest
...
def change_test(request, test_id):
	...
	if request.user.is_authenticated():
		if request.user.has_perm("hoopaloo_test.change_test"):
...
def test_view(request, test_id):
	...
	if request.user.is_authenticated():
		if request.user.has_perm(".see_tests"):
...
def annul_test(request, test_id):
	...
	if request.user.is_authenticated():
		if request.user.has_perm(".invalidate_tests"):
			if request.method == 'POST':
				test = queries.get_test(test_id)
				path = test.path
				exercise = queries.get_exercise(test.exercise.id)
