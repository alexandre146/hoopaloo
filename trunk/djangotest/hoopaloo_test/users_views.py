...
from djangotest.hoopaloo_test.models import Student, Assistant
from djangotest.hoopaloo_test.forms import RegisterAssistantForm, RegisterStudentForm, LoginForm, AssignStudentsForm
...
def register_student(request):
	...
	if request.user.is_authenticated():
		if request.user.has_perm("hoopaloo_test.add_student"):
			if request.method == 'POST':
...
def users(request):
	...
	if request.user.is_authenticated():
		if request.user.has_perm("hoopaloo_test.see_users"):
...
def delete_users(request, user_id):
	...
	if request.user.is_authenticated():
		if request.user.has_perm("hoopaloo_test.delete_user"):
