...
def make_message_student(result):
	...
	if result == None:
		...
		try:
			...
			#some error of import or error at execution
			return None, msgs
...
class Change_Availability():
	...
	def change_availability(self):
		...
		exercise.number_students_that_solved = queries.number_students_that_solved(exercise)
		exercise.save()
...
def copy_test_files(student):
	...
	for e in exercises:
		...
		try:
			...
			if contend.__contains__("from pexpect import *") or contend.__contains__("import pexpect"):
...
def save_under_test_file(test):
	...
	# save the new conted of test
	path_tests = settings.MEDIA_ROOT + '/under_tests/' 
	path_under_test = path_tests + test.path
	
	dest = open(path_under_test, 'wb')
	
	aux = "'" + path_tests + "'"
	if test.code.__contains__("from pexpect import *") or test.code.__contains__("import pexpect"):
		number = test.code.count('%s/')
		aux2 = number*(path_tests,)
		dest.write((test.code % aux2) + '\n\n\n' + (configuration.UNDER_TEST_COMPLEMENT % (test.name, aux)))
	else:
