...
from djangotest.hoopaloo_test.forms import *
import util
import queries
...
def submission(request):
	...
	if request.user.is_authenticated():
		if request.user.has_perm("hoopaloo_test.add_submission"):
			...
			if request.method == 'POST':
				...
				if form.is_valid():
					...
					r, m = util.make_message_student(result)
					if r:
						m.insert(0, configuration.SUBMISION_SUCESS)
						return render_to_response("submission.html", {'form' : form, 'msg' : m, 'submissions':submissions, 'student': student,}, context_instance=RequestContext(request))
					elif r == False:
						msg = [configuration.SUBMISION_SUCESS]
						return render_to_response("submission.html", {'form' : form, 'error' : m, 'msg':msg,'submissions':submissions, 'student': student,}, context_instance=RequestContext(request))
					else:
						error = []
						error.append(configuration.SUBMISSION_FAIL)
						return render_to_response("submission.html", {'form' : form, 'error' : error, 'submissions':submissions, 'student': student,}, context_instance=RequestContext(request))
				else:
					form = SubmissionForm() 
					error = []
...
def see_submissions(request):
	if request.user.is_authenticated():
		if request.user.has_perm(".see_submission"):
			student = queries.get_student_from_user_id(request.user.id)
			submissions = queries.get_ordered_student_submissions(student.id)
