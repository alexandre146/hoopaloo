...
class Exercise(models.Model):
	...
	def create_exercise(self, name, description, owner, available, date=None):
		from hoopaloo_test.util import get_time_unavailability, get_next_friday, Change_Availability
...
class Test(models.Model):
	...
	def create_test(self, owner, exercise, code):
		new_test = Test()
		new_test.name = 'Test_' + exercise.name.replace('.', '_')
		new_test.code = code
...
def create_or_update_test(sender, instance, signal, *args, **kwargs):
	...
	if not instance.locked:
		try:
			...
			test = instance
			from hoopaloo_test.Tester import Tester
